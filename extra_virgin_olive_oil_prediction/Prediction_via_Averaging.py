...
train_data = dfevoo[:int(len(dfevoo) * train_perc)]  # .sample(frac=0.8, random_state=0)
test_data = dfevoo.drop(train_data.index)
...
for di in range(0,smoothing_window_size*4,smoothing_window_size):
...
for pred_idx in range(window_size,N):
    ...
    if pred_idx >= N:
        priceStringDate = dt.datetime.strptime(k, '%Y-%m-%d').date() + dt.timedelta(days=1)
    else:
        priceStringDate = df.loc[pred_idx,'priceStringDate']

    std_avg_predictions.append(np.mean(train_data[pred_idx-window_size:pred_idx]))
    mse_errors.append((std_avg_predictions[-1]-train_data[pred_idx])**2)
    std_avg_x.append(priceStringDate)
...
for pred_idx in range(1,N):
    ...
    run_avg_x.append(priceStringDate)
...
# test_points_seq = np.arange(test_data.shape[0],dfevoo.shape[0],50).tolist()
...
#       # print(average_loss,valid_summary,train_seq_length, batch_size)
...
# plt.xlim(test_data.shape[0],dfevoo.shape[0])
#
# plt.subplot(2,1,2)
#
# # Predicting the best test prediction you got
# plt.plot(range(dfevoo.shape[0]),all_mid_data,color='b')
